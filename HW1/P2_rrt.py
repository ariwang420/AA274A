...
class RRT(object):
    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        state_dim = len(self.x_init) # if 3D then 3, etc.
        ...
        for k in range(1, max_iters+1):
            z = np.random.uniform(0,1) # sample uniform and if less than goal bias then go towards goal
            if z < goal_bias:
                x_rand = self.x_goal
            else: # otherwise go to a random point
                x_rand = np.random.uniform(self.statespace_lo, self.statespace_hi) # since both are 2D this works
                
            x_near_idx = self.find_nearest(V[:n,:], x_rand) # find the closest node
            x_near = V[x_near_idx, :] # this is the closest node
            
            x_new = self.steer_towards(x_near, x_rand, eps) # steer towards the node from closest by eps
            if self.is_free_motion(self.obstacles, x_near, x_new): # check for collisions
                V[n,:] = x_new
                P[n] = x_near_idx
                n+=1
                if np.linalg.norm(self.x_goal - x_new) < eps:
                    success = True
                    break
        
        if success:
            # reconstruct path by walking backward through parents
            path = [self.x_goal]
            curr = n - 1
            while curr != -1:
                path.append(V[curr, :])
                curr = P[curr]
            self.path = path[::-1]  # reverse from start → goal
    ...
    def shortcut_path(self):
        ...
        while not success:
            success = True
            # iterate backwards to avoid skipping
...
class GeometricRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        distances = np.linalg.norm(V - x, axis=1)  # shape: (N,)
