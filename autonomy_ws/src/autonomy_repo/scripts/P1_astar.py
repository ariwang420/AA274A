...
class AStar(object):
    ...
    def __init__(self, statespace_lo, statespace_hi, x_init, x_goal, occupancy, resolution=1):
        self.statespace_lo = statespace_lo         # state space lower bound (e.g., [-5, -5]) (x_low,y_low)
        self.statespace_hi = statespace_hi         # state space upper bound (e.g., [5, 5]) (x_high,y_high)
    ...
    def is_free(self, x):
        ...
        """
        ########## Code starts here ##########
        inside = (self.statespace_lo[0] <= x[0] <= self.statespace_hi[0] and 
                  self.statespace_lo[1] <= x[1] <= self.statespace_hi[1])
        return inside and self.occupancy.is_free(x)
        ########## Code ends here ##########
    ...
    def distance(self, x1, x2):
        ...
        """
        ########## Code starts here ##########
        return np.linalg.norm(np.array(x1)-np.array(x2)) #L2 dist
        #return np.linalg.norm(np.array(x1) - np.array(x2), ord=np.inf) # Chebyshev
        #return np.linalg.norm(np.array(x1) - np.array(x2), ord=1) # L1 dist
    ...
    def get_neighbors(self, x):
        ...
        for dx in [-1,0,1]:
            for dy in [-1,0,1]:
                if dx == 0 and dy == 0:
                    continue
                neigh_x = x[0] + dx*self.resolution
                neigh_y = x[1] + dy*self.resolution
                neigh_candidate = self.snap_to_grid((neigh_x, neigh_y))
                if self.is_free(neigh_candidate):
                    neighbors.append(neigh_candidate)
    ...
    def solve(self):
        ...
        while len(self.open_set)>0:
            ...
            for neigh in self.get_neighbors(x_current):
                if neigh in self.closed_set:
                    continue
                tent_cost_arr = self.cost_to_arrive[x_current] + self.distance(x_current, neigh)
                if neigh not in self.open_set:
                    self.open_set.add(neigh)
                elif tent_cost_arr > self.cost_to_arrive[neigh]:
                    continue
                self.came_from[neigh] = x_current
                self.cost_to_arrive[neigh] = tent_cost_arr
                self.est_cost_through[neigh] = tent_cost_arr + self.distance(self.x_goal, neigh)
            pass
        return False
        ########## Code ends here ##########
